...
import gnu.trove.impl.hash.THash; //l2jtw add
import gnu.trove.map.TIntObjectMap; //l2jtw add
import gnu.trove.map.hash.TIntObjectHashMap; //l2jtw add
...
public final class NpcStringId
{
	 ...
	 */
	//l2jtw add private static NpcStringId[] VALUES;
	...
	static TIntObjectHashMap<NpcStringId> nsIds; //l2jtw add
	private static final void buildFastLookupTable()
	{
		final Field[] fields = NpcStringId.class.getDeclaredFields();
		nsIds = new TIntObjectHashMap<>(fields.length); //l2jtw add
		...
		for (final Field field : fields)
		{
			...
			if (Modifier.isStatic(mod) && Modifier.isPublic(mod) && Modifier.isFinal(mod) && field.getType().equals(NpcStringId.class))
			{
				try
				{
					...
					nsIds.put(nsId.getId(), nsId); //l2jtw add
				}
	...
	{
		for (final NpcStringId nsId : nsIds.values()) //l2jtw add
		{
			if (nsId != null)
