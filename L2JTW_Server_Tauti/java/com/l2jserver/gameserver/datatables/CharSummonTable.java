...
public class CharSummonTable
{
	...
	private static final String SAVE_SUMMON = "INSERT INTO character_summons (ownerId,summonSkillId,curHp,curMp,time) VALUES (?,?,?,?,?)"; //l2jtw add
	private static final String LOAD_SUMMON = "SELECT curHp, curMp, time, summonSkillId FROM character_summons WHERE ownerId = ? limit 4"; //l2jtw add
	...
	public void restoreServitor(L2PcInstance activeChar)
	{
		...
		try
		{
			//l2jtw add int skillId = _servitors.get(activeChar.getObjectId());
			
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(LOAD_SUMMON);
			statement.setInt(1, activeChar.getObjectId());
			//l2jtw add statement.setInt(2, skillId);
			ResultSet rset = statement.executeQuery();
			...
			while (rset.next())
			{
				...
				int skillId = rset.getInt("summonSkillId"); //l2jtw add
		...
		removeServitor(activeChar); //l2jtw add
	}
