...
public final class L2PcInstance extends L2Playable
{
	...
	private int _recoBonusMode = 10; // Add recobonus by pmq //rocknow-God
	/** Recommendation task **/
	...
	private List<L2Summon> _summon = new FastList<>(); //l2jtw add
	...
	public void updatePvPFlag(int value)
	{
		...
		if (hasPet())
		{
			for(L2Summon s : getPets())
			{
				sendPacket(new RelationChanged(s, getRelation(this), false));
			}
		}
		...
		for (L2PcInstance target : plrs)
		{
			...
			if (hasPet())
			{
				for(L2Summon s : getPets())
				{
					target.sendPacket(new RelationChanged(s, getRelation(target), isAutoAttackable(target)));
				}
			}
		}
	}
	...
	public int giveAvailableSkills(boolean includedByFs, boolean includeAutoGet)
	{
		...
		while (skills.size() > unLearnable && tryLimit < 60) //rocknow-God (by otfnir)
		{
			...
			{
				if (!s.getRaces().isEmpty() && !s.getRaces().contains(race))
				{
					unLearnable++;
					continue;
				}
	...
	protected boolean checkDoCastConditions(L2Skill skill)
	{
		...
		switch (skill.getSkillType())
		{
			...
			case SUMMON:
			{
				if (!((L2SkillSummon)skill).isCubic() && (!canSummon(skill.getId()) || isMounted()))
				{
					if (Config.DEBUG)
		...
		for (L2PcInstance player : plrs)
		{
			...
			if (mov instanceof CharInfo)
			{
				...
				if (oldrelation != null && oldrelation != relation)
				{
					...
					for(L2Summon s : getPets())
					{
						player.sendPacket(new RelationChanged(s, relation, isAutoAttackable(player)));
					}
				}
			}
		}
	...
	{
		return _summon.size() > 0 ? _summon.get(0) : null;
	}

	@Override
	public List<L2Summon> getPets()
	{
		return _summon;
	}
	 ...
	 * @param summon 
	 */
	public void unsetPet(L2Summon summon)
	{
		_summon.remove(summon);
	}
	...
	public void broadcastKarma()
	{
		...
		for (L2PcInstance player : plrs)
		{
			...
			for(L2Summon s : getPets())
			{
				player.sendPacket(new RelationChanged(s, getRelation(player), isAutoAttackable(player)));
			}
		}
	}
	...
	private static L2PcInstance restore(int objectId)
	{
		...
		try
		{
			...
			L2PetInstance p = L2World.getInstance().getPet(player.getObjectId());
			if(p != null)
			{
				player.setPet(p);
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		try
		{
			...
			for(L2Summon s : getPets())
			{
				if (s instanceof L2ServitorInstance)
					s.unSummon(this);
			}
	...
	private synchronized void cleanup()
	{
		...
		CharSummonTable.getInstance().removeServitor(this);
		for(L2Summon s : getPets())
		{
			try
			{
	...
	public void teleportBookmarkAdd(int x,int y,int z,int icon, String tag, String name)
	{
		...
		if(tpbookmark.size() >= _bookmarkslot || tpbookmark.size() >= Config.MAX_BOOKMARKSLOT)
		{
		...
		for(int i = 1; i <= Config.MAX_BOOKMARKSLOT; i++)
		{
		...
		if(activeChar.isAwaken())
		{
			...
			switch(activeChar.getRace())
			{
				...
				case Orc:
					...
					break;
				case Dwarf:
					tempClassId = activeChar.isMageClass() ? ClassId.wynnSummoner : ClassId.dwarvenFighter; //temp fix
					break;
				case Kamael:
	...
	public boolean canSummon(int skillId)
	{
		int summonPoint = getSummonPoint(skillId);
		if(_summon.isEmpty() && summonPoint == 0)
		{
			return true;
		}
		...
		if(_summon.size() < 4)
		{
			if(summonPoint + getUsedSummonPoint() <= getMaxSummonPoint())
			{
				return true;
			}
		}
	...
	public int getMaxSummonPoint()
	{
		return (int) Math.round(calcStat(Stats.MAX_SUMMON_POINT, 0, null, null));
	}
	
	public int getSummonPoint(int skillId)
	{
		L2Skill skill = SkillTable.getInstance().getInfo(skillId, 1);
		if(skill != null && skill instanceof L2SkillSummon)
		{
			return ((L2SkillSummon)skill).getSummonPoint();
		}
		return 0;
	}
	
	public int getUsedSummonPoint()
	{
		int point = 0;
		for(L2Summon s : getPets())
		{
			if(s instanceof L2ServitorInstance)
			{
				int skillId = ((L2ServitorInstance) s).getReferenceSkill();
				point += getSummonPoint(skillId);
			}
		}
		//return (int) Math.round(calcStat(Stats.SUMMON_POINT, 0, null, null));
		return point;
	}
