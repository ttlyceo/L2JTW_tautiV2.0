			...
			int activationchance, int maxcount, int totallifetime, boolean givenByOther)
	{
		...
		switch (_id)
		{
			...
			case KNIGHT_CUBIC: //l2jtw add
				_skills.add(SkillTable.getInstance().getInfo(11292, 8));
				_skills.add(SkillTable.getInstance().getInfo(10056, 8));
				break;
		}
	...
	private class Action implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (_id >= SMART_CUBIC_EVATEMPLAR && _id <= KNIGHT_CUBIC) //l2jtw add
				{
				...
				else if (Rnd.get(1, 100) < _chance)
				{
					...
					if (skill != null)
					{
						...
						// l2jtw add start
						else if (skill.getId() == 10056 || skill.getId() == 11292) //l2jtw
						{
							_target = _owner;
						}
						// l2jtw add end
