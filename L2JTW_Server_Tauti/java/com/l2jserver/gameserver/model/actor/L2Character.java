...
import com.l2jserver.gameserver.network.serverpackets.ExTeleportToLocation;//488 Test
...
public abstract class L2Character extends L2Object
{
	...
	private final List<AttackListener> attackListeners = new FastList<AttackListener>().shared(); //l2jtw add
	private final List<DeathListener> deathListeners = new FastList<DeathListener>().shared(); //l2jtw add
	private final List<SkillUseListener> skillUseListeners = new FastList<SkillUseListener>().shared(); //l2jtw add
	...
	public void teleToLocation(int x, int y, int z, int heading, int randomOffset)
	{
		...
		broadcastPacket(new ExTeleportToLocation(this, x, y, z, heading));//488 Test
	...
	protected void doAttack(L2Character target)
	{
		...
		if (weaponItem != null)
			ssGrade = weaponItem.getItemGradeRPlus(); //l2jtw add
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		if (hitTime > 410)
		{
			...
			if (isPlayer() && !effectWhileCasting)
				sendPacket(new SetupGauge(0, hitTime)); //rocknow-God-Awaking
