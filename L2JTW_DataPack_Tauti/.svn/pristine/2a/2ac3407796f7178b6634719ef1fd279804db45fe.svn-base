...
REM 檢查 GS 支援的版本資訊
if not %vgs% == Tauti echo 無法繼續安裝 DP，因為：
if not %vgs% == Tauti echo GS 支援的版本是：%vgs%
if not %vgs% == Tauti echo DP 支援的版本是：Tauti
if not %vgs% == Tauti echo 請確定 GS 和 DP 都使用相同的版本後，再試一次
...
REM ------------------------------------------------------
REM 儲存 DP 支援的版本資訊
if %dp_err% == 0 echo 資料庫安裝未完成：Tauti> ..\doc\L2J_DataPack_Ver.txt
if %dp_err% == 1 echo Tauti> ..\doc\L2J_DataPack_Ver.txt
REM ------------------------------------------------------
call :colors 17
title L2JTW DataPack 安裝 - For：L2JTW GameServer Tauti Beta
cls
